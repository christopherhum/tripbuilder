...
<h2 style="text-align: center;"><strong>List of all Flights for Trip Number: {{ Session::get('current_trip') }}</strong></h2>
...
<table style="width: auto; margin-left: auto; margin-right: auto;" border="3" cellspacing="3" cellpadding="1">
    ...
    <tr>
        ...
        <td style="width: 150px;">
            ...
            <form action="{{url('removeFlight/' . $flight->id)}}" method="POST">
            <!-- Generate csrf input field token using blade method -->
                {{csrf_field()}}

                <!-- This blade method spoofs a request so DELETE can be picked up by Route -->
                {{ method_field('DELETE') }}
                <button type="submit" id="delete-trip-{{ $flight->id }}" style="width:auto;text-align:center;">
